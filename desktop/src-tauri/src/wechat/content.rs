...
impl WechatContentStore {
    pub(crate) fn new(data_dir: impl AsRef<Path>) -> Self {
        Self {
            root: data_dir.as_ref().join("wechat_reply").join("content"),
        }
    }
    ...
    pub(crate) fn delete_all(&self) -> Result<ContentDeleteResult, ContractError> {
        if !self.managed_root_exists()? {
            return Ok(ContentDeleteResult {
                deleted_request_directories: 0,
                failed_entries: 0,
            });
        }
        let mut result = ContentDeleteResult {
            deleted_request_directories: 0,
            failed_entries: 0,
        };
        for entry in fs::read_dir(&self.root).map_err(|_| ContractError::WxContentPersistFailed)? {
            ...
            let valid_request = name
                .to_str()
                .is_some_and(|value| uuid::Uuid::parse_str(value).is_ok());
            let metadata =
                fs::symlink_metadata(&path).map_err(|_| ContractError::WxContentPersistFailed)?;
    ...
    pub(crate) fn cleanup_expired(
        ...
        retention_enabled: bool,
        retention_days: u16,
    ) -> Result<ContentDeleteResult, ContractError> {
...
mod tests {
    ...
    fn disabled_retention_does_not_create_a_directory() {
        let directory =
            std::env::temp_dir().join(format!("wechat-content-{}", uuid::Uuid::new_v4()));
    ...
    fn cleanup_removes_only_expired_uuid_directories() {
        ...
        store
            .retain(&expired, WechatContentKind::OcrText, b"old", true, 1)
            .unwrap();
        store
            .retain(&current, WechatContentKind::OcrText, b"current", true, 1)
            .unwrap();
        ...
        fs::File::open(&expired_dir)
            ...
            .set_times(
                fs::FileTimes::new()
                    .set_modified(SystemTime::now() - StdDuration::from_secs(48 * 60 * 60)),
            )
            .unwrap();
    ...
    fn cleanup_refuses_a_symlinked_content_root() {
        ...
        let directory =
            std::env::temp_dir().join(format!("wechat-content-{}", uuid::Uuid::new_v4()));
        let outside =
            std::env::temp_dir().join(format!("wechat-content-outside-{}", uuid::Uuid::new_v4()));
