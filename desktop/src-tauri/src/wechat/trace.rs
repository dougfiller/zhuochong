...
use super::types::{BindingGeneration, BindingObservationVersion, CaptureVersion, ContractError, RequestId, SuggestionGeneration};
...
impl ReplyTraceEvent {
    pub(crate) fn stage(
        ...
        let terminal = matches!(state, ReplyState::ReplyReady | ReplyState::Cancelled | ReplyState::Failed);
        Self {
...
impl ReplyTraceStore {
    ...
    pub(crate) fn append(&self, event: ReplyTraceEvent) -> Result<(), ContractError> {
        ...
        let _writer = self.writer.lock().map_err(|_| ContractError::WxTracePersistFailed)?;
        fs::create_dir_all(&self.root).map_err(|_| ContractError::WxTracePersistFailed)?;
        let path = self.root.join(format!("{}.jsonl", event.occurred_at.format("%F")));
        let mut line = serde_json::to_vec(&event).map_err(|_| ContractError::WxTracePersistFailed)?;
        line.push(b'\n');
        ...
        file.write_all(&line).map_err(|_| ContractError::WxTracePersistFailed)?;
        file.sync_data().map_err(|_| ContractError::WxTracePersistFailed)
    }
    ...
    pub(crate) fn list(&self, query: TraceQuery) -> Result<WechatReplyTracePage, ContractError> {
        if query.limit == 0 || query.limit > MAX_PAGE_LIMIT
            || query.occurred_after.zip(query.occurred_before).is_some_and(|(after, before)| after >= before)
        {
        ...
        for path in trace_files(&self.root)? {
            let date = path.file_stem().and_then(|stem| stem.to_str()).ok_or(ContractError::WxTracePersistFailed)?;
                ...
                match parsed {
                    Ok(event) if complete => {
                        ...
                        if event_matches(&event, &query) {
                            records.push((CursorPosition { date: date.to_owned(), line: line_index }, event));
                        }
                    }
        ...
        Ok(WechatReplyTracePage { entries, next_cursor, tail_recovered })
    }
}
...
fn validate_event(event: &ReplyTraceEvent) -> Result<(), ContractError> {
    ...
    let terminal_stage = matches!(event.stage_name.as_str(), "reply_ready" | "cancelled" | "failed");
    let active_stage = matches!(event.stage_name.as_str(), "validating" | "capturing" | "ocr" | "retrieving" | "generating");
...
fn validate_m2_metadata(metadata: &M2TraceMetadata) -> Result<(), ContractError> {
    if metadata.catalog_generation_seq.is_some_and(|value| value == 0)
        || metadata.hit_ids.len() > MAX_HITS
        || metadata.hit_ids.len() != metadata.hit_scores.len()
        || !metadata.hit_ids.iter().all(|value| is_metadata_token(value))
        || !metadata.hit_scores.iter().all(|score| score.is_finite() && (0.0..=1.0).contains(score))
        || !metadata.index_generation_id.as_deref().is_none_or(is_metadata_token)
        || !metadata.model_request_id.as_deref().is_none_or(is_metadata_token)
        || !metadata.query_hmac.as_deref().is_none_or(is_sha256_hex)
        || !metadata.active_import_snapshot_hash.as_deref().is_none_or(is_sha256_hex)
...
fn event_matches(event: &ReplyTraceEvent, query: &TraceQuery) -> bool {
    query.request_id.as_ref().is_none_or(|request_id| event.request_id == request_id.to_string())
        && query.occurred_after.is_none_or(|after| event.occurred_at >= after)
        && query.occurred_before.is_none_or(|before| event.occurred_at < before)
}
...
fn trace_files(root: &Path) -> Result<Vec<PathBuf>, ContractError> {
    ...
    for entry in fs::read_dir(root).map_err(|_| ContractError::WxTracePersistFailed)? {
        let path = entry.map_err(|_| ContractError::WxTracePersistFailed)?.path();
        let valid_name = path.file_stem().and_then(|name| name.to_str()).and_then(|date| NaiveDate::parse_from_str(date, "%F").ok()).is_some();
        if path.extension().is_some_and(|extension| extension == "jsonl") && valid_name {
            paths.push(path);
        }
    }
    paths.sort();
...
struct CursorPosition { date: String, line: usize }
...
fn decode_cursor(value: &str) -> Result<TraceCursor, ContractError> {
    let decoded = URL_SAFE_NO_PAD.decode(value).map_err(|_| ContractError::WxTraceInvalidQuery)?;
...
mod tests {
    ...
    fn event(request_id: &RequestId, sequence: u64) -> ReplyTraceEvent {
        ReplyTraceEvent::stage(
            request_id, sequence, ReplyState::Validating, None,
            SuggestionGeneration::new(1), BindingGeneration::new(2), BindingObservationVersion::new(3), 0, None, None,
        )
    }
    ...
    fn append_paginates_and_recovers_a_truncated_tail() {
        ...
        let path = store.root.join(format!("{}.jsonl", Utc::now().format("%F")));
        OpenOptions::new().append(true).open(path).unwrap().write_all(b"{\"bad").unwrap();
        let first = store.list(TraceQuery { request_id: Some(request_id.clone()), occurred_after: None, occurred_before: None, cursor: None, limit: 1 }).unwrap();
        assert_eq!(first.entries.len(), 1);
        assert!(first.tail_recovered);
        let second = store.list(TraceQuery { request_id: Some(request_id), occurred_after: None, occurred_before: None, cursor: first.next_cursor, limit: 1 }).unwrap();
    ...
    fn rejects_mid_file_corruption() {
        ...
        assert!(matches!(
            store.list(TraceQuery { request_id: None, occurred_after: None, occurred_before: None, cursor: None, limit: 10 }),
    ...
    fn rejects_unbounded_or_invalid_metadata_and_event_shapes() {
        ...
        assert_eq!(validate_event(&invalid_event), Err(ContractError::WxTracePersistFailed));
