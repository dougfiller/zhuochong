...
pub(crate) fn model_profile_is_available(
    config: &WechatConfig,
...
mod tests {
    ...
    fn selected_model_requires_the_exact_successful_nonempty_profile() {
        ...
        assert_eq!(
            selected_verified_model(&config, &[profile.clone()])
                .unwrap()
                .model,
            "fixture"
        );
