...
mod tests {
    use super::super::types::{
        CaptureVersion, CapturedWechat, ContractError, NormalizedOcrText, OcrBackendResult,
        OcrReadyReply, RequestId,
    };
    ...
    fn fixture(name: &str) -> &'static str {
        match name {
            ...
            "duplicate_message" => {
                include_str!("../../tests/fixtures/wechat_contract/duplicate_message.json")
            }
            "unsupported_schema" => {
                include_str!("../../tests/fixtures/wechat_contract/unsupported_schema.json")
            }
            "ambiguous_conversations" => {
                include_str!("../../tests/fixtures/wechat_contract/ambiguous_conversations.json")
            }
            "no_hit" => include_str!("../../tests/fixtures/wechat_contract/no_hit.json"),
            "retrieval_failed" => {
                include_str!("../../tests/fixtures/wechat_contract/retrieval_failed.json")
            }
            _ => unreachable!(),
        }
    }
    ...
    fn ocr_fixtures_preserve_the_input_boundary() {
        ...
        assert!(OcrReadyReply::from_backend(
            captured,
            OcrBackendResult::Text(NormalizedOcrText::parse(&normal.ocr_text).unwrap())
        )
        .is_ok());

        let empty: OcrFixture = serde_json::from_str(fixture("empty_ocr")).unwrap();
        assert_eq!(
            NormalizedOcrText::parse(&empty.ocr_text),
        ...
        assert_eq!(
            OcrReadyReply::from_backend(
                captured,
                OcrBackendResult::Text(NormalizedOcrText::parse(&group.ocr_text).unwrap())
            ),
