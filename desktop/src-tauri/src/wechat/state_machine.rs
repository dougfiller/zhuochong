...
impl StateMachine {
    ...
    pub(crate) fn state(&self) -> ReplyState {
        self.state
    }
    ...
    pub(crate) fn mode(&self) -> ReplyMode {
        self.mode
    }

    pub(crate) fn stage_seq(&self) -> u64 {
        self.stage_seq
    }
    ...
    pub(crate) fn observation_version(&self) -> BindingObservationVersion {
    ...
    fn is_legal(&self, next: ReplyState) -> bool {
        ...
        match (self.state, next) {
            (Idle, Validating) | (Validating, Capturing) | (Capturing, Ocr) => true,
...
mod tests {
    ...
    fn m1_cannot_retrieve_and_m2_must_retrieve() {
        ...
        m1.advance(ReplyState::Validating, 1).unwrap();
        m1.advance(ReplyState::Capturing, 2).unwrap();
        m1.advance(ReplyState::Ocr, 3).unwrap();
        assert_eq!(
            m1.advance(ReplyState::Retrieving, 4),
            Err(ContractError::WxContractViolation)
        );
        m1.advance(ReplyState::Generating, 4).unwrap();

        let mut m2 = machine(ReplyMode::M2);
        m2.advance(ReplyState::Validating, 1).unwrap();
        m2.advance(ReplyState::Capturing, 2).unwrap();
        m2.advance(ReplyState::Ocr, 3).unwrap();
        assert_eq!(
            m2.advance(ReplyState::Generating, 4),
            Err(ContractError::WxContractViolation)
        );
        m2.advance(ReplyState::Retrieving, 4).unwrap();
        assert_eq!(
            m2.advance(ReplyState::Generating, 5),
            Err(ContractError::WxContractViolation)
        );
    }
    ...
    fn stage_sequence_is_strict_and_stale_m2_result_is_rejected() {
        ...
        assert_eq!(
            state.advance(ReplyState::Capturing, 1),
            Err(ContractError::WxContractViolation)
        );
    ...
    fn ocr_failures_terminate_before_model_or_retrieval_stages() {
        for error in [
            ...
            assert_eq!(
                state.advance(ReplyState::Retrieving, 5),
                Err(ContractError::WxContractViolation)
            );
            assert_eq!(
                state.advance(ReplyState::Generating, 5),
                Err(ContractError::WxContractViolation)
            );
        }
    }
